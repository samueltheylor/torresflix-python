...
class TorresFlixRegressionTests(unittest.TestCase):
    ...
    def test_list_and_likes_reject_unknown_movie_ids(self):
        ...
        for path in ("/api/toggle-list", "/api/toggle-like"):
            self.assertEqual(client.post(path, json={"movie_id": 99999}).status_code, 400)
    ...
    def test_ratings_are_private_to_each_session(self):
        ...
        self.assertEqual(first.post("/api/rate", json={"movie_id": 1, "rating": 5}).status_code, 200)
    ...
    def test_rating_must_be_in_range(self):
        client = app.test_client()
        self.login(client)
        self.assertEqual(client.post("/api/rate", json={"movie_id": 1, "rating": 6}).status_code, 400)
